...
2. Variable resolution is computed with the active-environment overlay, request-level overrides, and auth/runtime mapping when enabled.

3. `RequestBuilder` assembles the final method, URL, headers, body, and auth after inherited auth, request variables, and script mutations are applied.

4. Scripts run for the current collection/folder/request lifecycle.

5. OAuth2 values are injected when enabled, using the active environment profile as the source of truth for token outputs and callback state.

6. Request is built and sent.

7. Response, script output, History, and Diagnostics are updated.
...
The runner is sequential. Each request passes through the same shared pipeline used by Workbench Send, including auth inheritance, variable resolution, script lifecycle execution, and OAuth2/runtime overlays.
...
`HistoryStore` retains the latest 1,000 entries by default, `HistoryPersistenceService` stores them with the workspace, and `HistoryExportService` and its format-specific services produce HAR/JSON/CSV output. `HistorySanitizer` handles safe text normalization and CSV-cell/formula safety; it is not a secret-redaction engine.
...
`DiagnosticStore` keeps a passive, bounded event list. `DiagnosticEvent`, `DiagnosticOperation`, `DiagnosticSeverity`, `DiagnosticSink`, and `DiagnosticSanitizer` support grouped reports with warning, error, and debug summaries. Raw events are retained first; sanitization is applied when reports or snapshots are generated.
