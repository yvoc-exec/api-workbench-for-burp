...
`HistoryStore` enforces hard entry and canonical retained-byte budgets (1,000 entries and 100 MiB by default). Pinned evidence counts toward the same limits. Admission evicts the oldest eligible unpinned evidence first and rejects an incoming capture atomically when protected pinned evidence exhausts the budget; rejection does not fail the Workbench request or Runner attempt. Legacy workspaces missing the History policy version may compact payloads once, preserving original lengths and SHA-256 hashes and marking the affected entries. Current-version ordinary mutations never compact pinned evidence to force acceptance. `HistoryPersistenceService` stores the bounded state with the workspace, and `HistoryExportService` and its format-specific services produce HAR/JSON/CSV output. Clearing History does not imply physical Burp project-page reclamation, and lightweight History UI ownership remains future work.
