...
| Governance / CI | `GOOD` | `ui-tests`, `performance-tests`, `static-analysis`, `mutation-tests` profiles and the consolidated `Build & Validate API Workbench JAR` workflow | Linux/Windows compatibility matrix, Xvfb UI job, SpotBugs check, PIT coverage run, JaCoCo enforcement, and canonical JAR artifact gating after full validation. | PIT still runs long; current enforced threshold is a non-regression floor, not the aspirational target. |
